...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/register", registrationRateLimit, async (req, res) => {
    try {
      ...
      const { generateVerificationCode, sendVerificationEmail } = await import('./services/emailVerification');
      const verificationCode = generateVerificationCode();
      const verificationExpiry = new Date(Date.now() + 60 * 60 * 1000); // 1 hour from now
      ...
      const user = await storage.createUser({
        ...
        termsAcceptedAt: new Date() // User accepted terms during registration
      });
  ...
  app.post("/api/auth/verify-email", authRateLimit, async (req, res) => {
    try {
      ...
      const { sendWelcomeEmail } = await import('./services/emailVerification');
      ...
      try {
        ...
        console.error('[Verification] Failed to send welcome email:', emailError);
      }
  ...
  */
  ...
  app.post("/api/orders", async (req, res) => {
    try {
      ...
      // Use frontend-calculated pricing (already validated by client)
      // DON'T recalculate pricing on backend - trust the frontend calculation

      // Calculate estimated delivery time and distance
      let routeEstimate = null;
      if (req.body.pickupCoordinates && req.body.returnCoordinates) {
        const { estimateRoute } = await import('@shared/routeEstimator');
        routeEstimate = estimateRoute(
          req.body.pickupCoordinates.lat,
          req.body.pickupCoordinates.lng,
          req.body.returnCoordinates.lat,
          req.body.returnCoordinates.lng
        );
      }
      ...
      orderData = {
        ...
        itemDescription,
        estimatedWeight,
        returnCoordinates: req.body.returnCoordinates,
        ...
        ...(routeEstimate && {
          routeDistanceMiles: routeEstimate.distanceMiles,
          estimatedDurationMinutes: routeEstimate.estimatedMinutes,
          timeCapMinutes: routeEstimate.timeCapMinutes,
          estimatedDeliveryTime: new Date(Date.now() + routeEstimate.estimatedMinutes * 60 * 1000)
        })
        
        // Pricing fields already in orderData from req.body (frontend calculation)
      };
